...
app.put("/events/:id", async (req, res) => {
  ...
  db.query(sql, [name, description, startTime, endTime, visible, id], (err, results) => {
    if (err) {
      return res.status(400).json({
        events: [],
...
app.post("/menus", (req, res) => {
  const { menuData, date, visible } = req.body;
  const sql = "INSERT INTO menu (MenuData, Date, IsVisible) VALUES (?, ?, ?)";
  db.query(sql, [JSON.stringify(menuData), date, visible], (err, result) => {
    if (err) {
      return res.status(400).json({
...
app.put("/menus/:menuId", (req, res) => {
  ...
  db.query(sql, [JSON.stringify(menuData), date, visible, menuId], (err, result) => {
    if (err) {
      return res.status(400).json({
